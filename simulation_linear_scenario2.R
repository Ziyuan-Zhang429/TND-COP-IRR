...
output_dir <- paste0("subtask_", subtask_num, "_bus")
...
run_simulation <- function(sim_num, subtask_num) {
  ...
  for (t in 1:sim_days) {
      ...
      for (node in 1:nrow(S_nodes)) {
        inf <- rbinom(1, 1, 
                      min(1, max(0, if (S_nodes$high_risk[node] == 1) {
                        high_risk_multiplier * beta * (1 - S_nodes$immunity_model[node]) * (nrow(I_nodes) / n)
                      } else {
                        beta * (1 - S_nodes$immunity_model[node]) * (nrow(I_nodes) / n)
                      }))
        )
        if (inf == 1) {
      ...
      for (node in 1:nrow(R_nodes)) {
        inf <- rbinom(1, 1, 
                      min(1, max(0, if (R_nodes$high_risk[node] == 1) {
                        high_risk_multiplier * beta * (1 - R_nodes$immunity_model[node]) * (nrow(I_nodes) / n)
                      } else {
                        beta * (1 - R_nodes$immunity_model[node]) * (nrow(I_nodes) / n)
                      }))
        )
        if (inf == 1) {
    ...
    if (t %in% investigate_date) {
      ...
      filename <- sprintf("simulation_subtask_%d_sim_%d_day_%d.csv", subtask_num, sim_num, t)
